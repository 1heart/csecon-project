...
For example, let us define a competence measure $C_i \in \mathbb{R}$ for $ i \in V$, which is the objective measure of a voter $i$'s competence in voting, and $c_ij$ for $i, j \in V$ for the perceived competence measure of voter $j$ perceived by voter $i$. If we assume that $i$ will 
